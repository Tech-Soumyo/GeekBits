...
class Solution{
    public:
        int totalTime(int n, vector<int> &arr, vector<int> &time)
        {
            ...
            int ans = -1;                                // Ans is initialised -1 becuz of, for first time entry of an element
            for(int i =0; i<n ; i++)
            {
                ans++;                                   // For Every Traversing nned to increment the Ans 
                if(mp[arr[i]])                           // Check whether arr[i] is in mp or not
                {
                    ans += time[arr[i]-1];               // Add the time of the element in ans 
                    ans--;
                }
                mp[arr[i]]++;                            // if arr[i] was not in the mp then push the arr[i] into mp
            }
            return ans;
        }
};